...
use eimzo_keys::{Discovery, DiscoveryConfig, KeyKind, Sessions};
use eimzo_rpc::dispatch::{Dispatcher, DispatcherConfig};
use eimzo_rpc::origin::{ApikeyConfig, ApikeyService, ApikeyStore};
use eimzo_rpc::ui::UiBroker;
use eimzo_rpc::Lang;
use eimzo_server::{Server, ServerConfig, ServerEvent, ServerHandle};
...
/// `config`'s own `extra_folders`. `list_disks` (`eimzo_keys::Discovery`)
...
/// in `eimzo-rpc`) needing to become mutable just to serve this one path.
...
fn key_entry_from_info(info: eimzo_keys::KeyInfo) -> KeyEntry {
    let now_ms = eimzo_pki::x509::epoch_millis(std::time::SystemTime::now());
...
fn map_pki_error(e: eimzo_pki::PkiError) -> EngineError {
    match e {
        eimzo_pki::PkiError::PasswordIncorrect => EngineError::Password,
        other => {
...
impl Engine {
    ...
    pub fn new(
        ...
        // (`crates/eimzo-rpc/src/plugins/randseed.rs`'s own `get`/`consent`).
    ...
    pub async fn status(&self) -> EngineStatus {
        ...
        let tls_trusted = if wss && has_cert {
            self.runtime.spawn(eimzo_server::tls::probe_trust(wss_port)).await.unwrap_or(false)
        } else {
            false
        };
    ...
    /// keeping every alias and chain: `eimzo_pki::pkcs12`/`ytks`'s own
    /// `read`/`write` round trip, exactly what `eimzo-cli`'s
    /// `PfxChangePassword` and its YKS analog already do, run off the
    /// runtime. The rewrite itself goes through
    /// `eimzo_rpc::plugins::keystore::overwrite_key_file` — the same
    ...
    pub async fn change_password(&self, path: String, old: String, new: String) -> Result<(), EngineError> {
        ...
        let bytes = self
            ...
            .spawn_blocking(move || -> std::result::Result<Vec<u8>, EngineError> {
                ...
                if is_pfx {
                    let store = eimzo_pki::pkcs12::read(&data, &old).map_err(map_pki_error)?;
                    eimzo_pki::pkcs12::write(&store, &new, &mut rand_core::OsRng).map_err(map_pki_error)
                } else if is_yks {
                    let store = eimzo_pki::ytks::read(&data, &old).map_err(map_pki_error)?;
                    eimzo_pki::ytks::write(&store, &new, &mut rand_core::OsRng).map_err(map_pki_error)
                } else {
        ...
        // `eimzo_rpc::plugins::keystore::blocking`), which panics ("there is no reactor
        ...
        self.runtime
            .spawn(async move { eimzo_rpc::plugins::keystore::overwrite_key_file(&path_buf, &bytes).await })
            .await
            .map_err(|e| {
    ...
    pub async fn convert_pfx_to_yks(&self, path: String, password: String) -> Result<(), EngineError> {
        ...
        let yks = self
            ...
            .spawn_blocking(move || -> std::result::Result<Vec<u8>, EngineError> {
                ...
                let store = eimzo_pki::pkcs12::read(&data, &password).map_err(map_pki_error)?;
                let now_ms = chrono::Utc::now().timestamp_millis();
                eimzo_pki::ytks::write(&eimzo_pki::ytks::from_pkcs12(&store, now_ms), &password, &mut rand_core::OsRng)
        ...
        self.runtime
            .spawn(async move { eimzo_rpc::plugins::keystore::create_key_file_exclusive(&output, &yks).await })
            .await
            .map_err(|e| {
    ...
    pub async fn convert_yks_to_pfx(&self, path: String, password: String) -> Result<(), EngineError> {
        ...
        let pfx = self
            ...
            .spawn_blocking(move || -> std::result::Result<Vec<u8>, EngineError> {
                ...
                let store = eimzo_pki::ytks::read(&data, &password).map_err(map_pki_error)?;
                eimzo_pki::pkcs12::write(&eimzo_pki::ytks::to_pkcs12(&store), &password, &mut rand_core::OsRng)
                    .map_err(map_pki_error)
            })
            .await
            .map_err(|e| {
        ...
        // rather than awaited bare, since `create_key_file_exclusive` needs a live reactor.
        self.runtime
            .spawn(async move { eimzo_rpc::plugins::keystore::create_key_file_exclusive(&output, &pfx).await })
            .await
            .map_err(|e| {
    ...
    /// enforced by `eimzo_pki::qrkey::export` itself.
    pub async fn export_qr_key(&self, path: String, password: String) -> Result<Vec<u8>, EngineError> {
        ...
        self.runtime
            .spawn_blocking(move || -> std::result::Result<Vec<u8>, EngineError> {
                ...
                eimzo_pki::qrkey::export(&key.private_key, &password).map_err(map_pki_error)
            })
            .await
            .map_err(|e| {
    ...
    pub async fn unlock_key(&self, path: String, alias: String, password: String) -> Result<CertificateSummary, EngineError> {
        ...
        self.runtime
            .spawn_blocking(move || -> std::result::Result<CertificateSummary, EngineError> {
                ...
                let view = eimzo_pki::x509::certificate_view(cert).map_err(|e| {
                    tracing::debug!(error = %e, "unlock_key: could not read certificate");
                    EngineError::KeyFile
                })?;
                let expired = eimzo_pki::x509::system_time(&cert.tbs_certificate.validity.not_after) < std::time::SystemTime::now();
    ...
    /// merged by `ApikeyService::sites` in `eimzo-rpc`'s origin gate.
