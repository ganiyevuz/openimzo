...
//! that lets the foreign `UiDelegate` stand in for `openimzo_rpc::ui::UiDelegate`
...
//! named type in `openimzo_rpc::ui`, which carries different fields (it has no
...
/// in `openimzo-rpc` already serializes these and applies the deadline, so an
...
pub trait Platform: Send + Sync {
    ...
    /// (`openimzo_rpc::plugins::randseed`) treats this as an opaque blob, same
    /// as `openimzo-cli`'s own `CliRandseedProvider`. A synchronous method,
...
/// Adapts `Platform::network_description` to `openimzo_rpc`'s own
/// `RandseedProvider`, so the macOS build has a real answer for
/// `randseed.get` — `openimzo-cli` wires its own `CliRandseedProvider` for the
...
/// unnecessary dependency of `openimzo-ffi` (or, worse, copy-pasting the CLI's
...
/// `openimzo_rpc::plugins::randseed`, so the two can never diverge on the wire
...
impl openimzo_rpc::plugins::randseed::RandseedProvider for PlatformRandseedProvider {
    fn network_description(&self) -> Vec<u8> {
...
/// (`openimzo_rpc::ui::REQUEST_TIMEOUT`), so the panels' countdowns can never
/// drift from the deadline the broker itself actually enforces.
fn deadline_secs() -> u32 {
    openimzo_rpc::ui::REQUEST_TIMEOUT.as_secs() as u32
}

/// 16 random bytes, hex-encoded — the same shape `openimzo_keys::Sessions` uses
...
/// Makes the foreign `UiDelegate` satisfy `openimzo_rpc::ui::UiDelegate`. The
...
#[async_trait]
impl openimzo_rpc::ui::UiDelegate for UiDelegateAdapter {
    async fn ask_password(
        &self,
        request: openimzo_rpc::ui::PasswordRequest,
        ...
        match self.ui.ask_password(foreign_request).await {
            ...
            Some(answer) => {
                Ok(openimzo_rpc::ui::PasswordAnswer { password: Zeroizing::new(answer.password), remember: answer.remember })
            }
            None => Err(openimzo_rpc::ui::UiError::Cancelled),
        }
    }

    async fn ask_permission(
        &self,
        request: openimzo_rpc::ui::PermissionRequest,
        ...
        Ok(match answer {
            PermissionAnswer::AllowOnce => openimzo_rpc::ui::PermissionAnswer::AllowOnce,
            PermissionAnswer::AllowAlways => openimzo_rpc::ui::PermissionAnswer::AllowAlways,
            PermissionAnswer::Deny => openimzo_rpc::ui::PermissionAnswer::Deny,
        })
    }
    ...
    async fn ask_new_pfx(
        ...
        match self.ui.ask_new_pfx(foreign_request).await {
            Some(answer) => {
                Ok(openimzo_rpc::ui::NewPfxAnswer { disk: answer.disk, password: Zeroizing::new(answer.password) })
            }
            None => Err(openimzo_rpc::ui::UiError::Cancelled),
        }
    }
    ...
    async fn confirm_legacy_algorithm(
        &self,
        request: openimzo_rpc::ui::LegacyAlgRequest,
        ...
        let use_suggested = self
            ...
            .await;
        Ok(if use_suggested { openimzo_rpc::ui::LegacyAlgAnswer::UseSuggested } else { openimzo_rpc::ui::LegacyAlgAnswer::KeepRequested })
    }

    async fn confirm_randseed(
        &self,
        request: openimzo_rpc::ui::RandseedRequest,
        ...
        Ok(if allow { openimzo_rpc::ui::Consent::Allow } else { openimzo_rpc::ui::Consent::Deny })
    }
