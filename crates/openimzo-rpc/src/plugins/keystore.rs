...
use openimzo_keys::session::{PASSWORD_TTL_REMEMBERED, PASSWORD_TTL_TRANSIENT};
use openimzo_keys::{KeyInfo, SessionType};
use openimzo_pki::pkcs12;
use openimzo_pki::ytks::{self, YtksEntry};
use rand::RngCore;
use std::io::Write;
...
///
/// `pub`, not `pub(super)` like the rest of this module: `openimzo-ffi`'s own
...
/// `openimzo-ffi`'s `engine.rs`, write their converted output through this.
///
...
///
/// The temporary file is created with owner-only permissions from the
/// moment it exists, matching `openimzo-server`'s own `write_atomic` for the
...
/// Not `openimzo_pki::PkiError` directly: the closure also covers the file
...
/// `openimzo_pki::PkiError` to `OpenError`, for the parse step of an
...
/// would produce.
pub(super) fn open_error(e: openimzo_pki::PkiError) -> OpenError {
    match e {
        openimzo_pki::PkiError::PasswordIncorrect => OpenError::WrongPassword,
...
fn require_signed_by(child: &Certificate, issuer: &Certificate, what: &'static str) -> Result<()> {
    if openimzo_pki::x509::verify_certificate_signature(child, issuer)? {
        Ok(())
    } else {
...
pub(super) fn check_key_matches_certificate(private: &PrivateKey, subject_certificate: &Certificate) -> Result<()> {
    ...
    let cert_key = openimzo_pki::x509::public_key(subject_certificate)?;
