...
use openimzo_keys::{Discovery, Sessions};
...
/// installer (`openimzo-ffi`) only ever sends into a broadcast channel, which
/// never blocks.
...
pub struct DispatcherConfig {
    ...
    /// `randseed.get`. `openimzo-rpc` may not read the environment itself, so
