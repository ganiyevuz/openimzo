...
use eimzo_pki::pkcs12::{self, KeyEntry, Pkcs12Store};
use eimzo_pki::{tempchain, x509};
use rand_core::OsRng;
...
async fn list_all_certificates(ctx: &Ctx, _origin: &Origin, _request: &Request) -> Result<Response> {
    let discovery = ctx.discovery.clone();
    let list = keystore::blocking(move || -> Result<Vec<eimzo_keys::KeyInfo>> { Ok(discovery.scan(KeyKind::Pfx)) }).await?;
    keystore::certificates_response(list)
}
...
async fn save_temporary_pfx(ctx: &Ctx, _origin: &Origin, request: &Request) -> Result<Response> {
    ...
    // that check and this write, the same way `eimzo-ffi`'s converters do.
