...
impl ApikeyService {
    pub fn new(config: ApikeyConfig, store: Box<dyn ApikeyStore>) -> Self {
        ...
        let verified = store
            ...
            .into_iter()
            .filter(|(domain, key)| {
                let ok = eimzo_crypto::apikey::verify_domain(domain, key);
                if !ok {
    ...
    /// check (`eimzo_crypto::apikey::verify_domain`) runs, so callers such as
    /// the main plugin's `apikey` function never repeat it.
    pub fn register(&self, domain: &str, key: &str) -> bool {
        if !eimzo_crypto::apikey::verify_domain(domain, key) {
            return false;
        }
    ...
    /// in `eimzo-ffi` only needs to convert each row into its own `Site`
